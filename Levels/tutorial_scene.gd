...
func _on_goblin_interact_player_interacted() -> void:
	if goblinInteraction:
		print("goblin interact called")
		...
		var tween = create_tween()
		tween.tween_property($InteractInstructions, "visible_characters", $InteractInstructions.text.length(), TextSpeed * $InteractInstructions.text.length())
		await get_tree().create_timer(TextSpeed * $InteractInstructions.text.length() + 0.5).timeout
		$InteractInstructions.visible_characters = 0
		$InteractInstructions.text = "Carful! If you interact before it starts flashing you will lose coins too!"
		var tween2 = create_tween()
		tween2.tween_property($InteractInstructions, "visible_characters", $InteractInstructions.text.length(), TextSpeed * $InteractInstructions.text.length())
		await get_tree().create_timer(TextSpeed * $InteractInstructions.text.length() + 0.5).timeout
		$Player.canMove = true

func _on_angel_detection_body_entered(body: Node2D) -> void:
	goblinInteraction = false
	var tween = create_tween()
...
func _on_false_start_detection_body_entered(body: Node2D) -> void:
	var tween = create_tween()
	tween.tween_property($FalseStartInstructions, "visible_characters", $FalseStartInstructions.text.length(), TextSpeed * $FalseStartInstructions.text.length())
